...
class DDPGModel(base_model.BaseModel):
    ...
    def __build_critic(state, action, scope, phase, trainable=True):
        ...
        with tf.variable_scope(scope):
            ...
            # batch_norm_action = tf.contrib.layers.batch_norm(action, center=True, scale=True, is_training=phase,
            # trainable=trainable)
...
if __name__ == "__main__":
    ...
    dir_conf, agent_config = conf["dir_config"], conf["ddpg"]
    ...
    for i in range(training_conf["num_agents"]):
        agt = DDPGAgent("DDPG_" + str(i), agent_config)
            ...
            for time in range(training_conf["max_round"]):
                ...
                for index, player in enumerate(agent_list):
                    ...
                    action = player.choose_action(phi_curr_state,
                                                  UPPER_BOUND)
                    action = int(action)
                    if action < agent_config["action_lower_bound"]:
                        action = agent_config["action_lower_bound"]
